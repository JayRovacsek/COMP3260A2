...
The program require an internet connection to compile, as some of the librarys are are retrieved from github (This is how rust's cargo system works)
It may be run with the command: `cargo run <infile> <outfile>` (for faster run-time you may put the `--release` flag directly after `cargo run`)
such that:
...
application.rs handles file input/output and the command line argument input/std output.
mod.rs handles des encryption/decryption and the avalanche effect analysis.
